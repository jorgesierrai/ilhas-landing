...
1. ~~**Se está publicando metadato interno.** `src/components/medios/VideoClip.astro` mete `slot.duracion` en el `<figcaption>`.~~ ✅ **Arreglado** en `fix-comentarios-2-de-assets`: el pie se quedó solo con `slot.caption`. Efecto lateral a revisar: `nosotros-conferencia-clip` no tiene `caption` en el manifiesto, así que ahora se queda **sin pie**. Si necesita uno, escríbelo en `src/data/medios.ts`.
